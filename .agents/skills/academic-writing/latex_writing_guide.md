...
\usepackage{fontawesome5}        % Icon font — \faIcon{calendar}, \faLightbulb, ...
\usepackage{vipythonhighlight}   % Custom package AIO — highlight Python
```

> **Lưu ý:** `fontawesome5` cần thiết cho icon trong `guidelinebox` (`\faLightbulb`), `notebox` (`\faInfoCircle`), và bảng data types (`\faIcon{calendar}`, `\faIcon{globe}`, ...). **KHÔNG được bỏ package này.**
...
### Kiểu 2: Caption viền xanh nền sáng (có minipage) — `figurecaptionmodern`

Dùng khi: hình đã có nhiều màu sắc, caption dài cần `\raggedright`.
...
### Kiểu 3: Caption viền xanh nền sáng (KHÔNG minipage) — `figurecaptionmodern` ★

> ★ **Đây là kiểu được dùng phổ biến nhất** trong bài viết thực tế (ví dụ `main_tableau.tex`). Đơn giản hơn Kiểu 2 vì bỏ `minipage` wrapper — `figurecaptionmodern` tự co theo `width=\linewidth`.
...
\begin{figurecaptionmodern}
        \capfig{Cửa sổ Calculated Field --- các thành phần chính gồm: (1) ô nhập tên field, (2) vùng soạn code, (3) indicator lỗi, và (4) danh sách hàm phân loại.}
        \label{fig:calculated_field}
    \end{figurecaptionmodern}
\end{figure}
```

### Kiểu 4: Caption truyền thống (đơn giản)
...
- Caption ngắn (1 dòng) → `\centering` (hoặc bỏ qua — mặc định đã center)
...
- Thư mục hình: có thể dùng `Figures/` hoặc `images/` tùy project — nhưng phải nhất quán trong cùng bài
...
### Kiểu 7: Bảng `tabularx` + `booktabs` (KHÔNG tcolorbox) ★

> ★ **Đây là kiểu được dùng phổ biến nhất** trong bài viết thực tế (ví dụ `main_tableau.tex`). Gọn nhẹ, không cần tcolorbox wrapper, chỉ dùng `booktabs` rules (`\toprule`, `\midrule`, `\bottomrule`).
...
\toprule
\textbf{Icon} & \textbf{Data Type} & \textbf{Mô tả} & \textbf{Ví dụ} \\
\midrule
\textbf{Abc} & \textbf{String} & Văn bản --- chuỗi ký tự bất kỳ & \texttt{Customer Name} \\
\textbf{\#} & \textbf{Number} & Số --- nguyên hoặc thập phân & \texttt{Sales}, \texttt{Profit} \\
\bottomrule
\end{tabularx}
\end{center}
```

> **Khi nào dùng Kiểu 7 thay vì Kiểu 1?**
> - Bảng tra cứu nhanh (quick reference), bảng so sánh ngắn → Kiểu 7
> - Bảng kết quả chính thức (evaluation, benchmark) cần caption/đánh số → Kiểu 1
...
\vspace{-3.2em}  % Loại bỏ khoảng trống giữa section heading và danh sách tham khảo
\printbibliography
```

> **Lưu ý:** `\vspace{-3.2em}` trước `\printbibliography` là để loại bỏ khoảng trống thừa phía dưới tiêu đề section "Tài liệu tham khảo" — bởi vì `\printbibliography` tự nó đã tạo heading riêng (được suppress bởi `\AtBeginDocument{\renewcommand\refname{}}` trong preamble).
...
\section{Giới thiệu}          % I.
\label{section:intro}
% ... nội dung giới thiệu chủ đề ...

\newpage
\setcounter{tocdepth}{2}       % Chỉ hiện đến subsection trong TOC
\tableofcontents               % Mục lục
\thispagestyle{fancy}

\newpage
\section{Nội dung chính 1}     % II.
\label{section:main1}
% ... conceptbox, guidelinebox, notebox, hình, bảng, code ...

\newpage
\section{Nội dung chính 2}     % III.
\label{section:main2}
% ... tiếp tục nội dung ...
...
> **Lưu ý:** Cấu trúc section linh hoạt — không bắt buộc phải có "Bài tập" hay "Câu hỏi trắc nghiệm". Quan trọng nhất là:
> 1. Bắt đầu bằng section Giới thiệu
> 2. Có `\tableofcontents` sau giới thiệu
> 3. Kết thúc bằng Tài liệu tham khảo + Phụ lục
